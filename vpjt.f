...
*
       include 'pjt.f'
*
       complex alpha(ny2),z(ny2,ny2),kappa(ny2)
       real beta(ny2),wk(iwk)
       integer ier,ia,irow
*
* init prepares the U, dQ/dy, dB/dy profiles
       call inijet
       do 2 iB=1,nB
       bet=bet0+dB*float(iB-1)
       do 5 ik=1,nk
       xk=xk0+dk*float(ik-1)
* formab creates the A and B matrices
       call formab
* eigzf computes the eigenvalues and eigenvectors
* of the generalized A X = c B X eigenproblem
       call eigzf(A,ny2,B,ny2,ny2,ijob,alpha,beta,z,ny2,wk,ier)
       do 3 ia=1,ny
       if(beta(ia).ne.0.0) kappa(ia)=alpha(ia)/beta(ia)
3      continue
* now identify the eignevalue with the largest
* imageinary part
       ci=0.0
       do 4 ia=1,ny
...
4      continue
* now we have the phase speed cr and the growth rate tau
* print them
       cr=real(kappa(irow))
       tau=ci*xk
...
c ***************************************
       ...
       dQ1(iy)=dQ1(iy)+F1*U1(iy)+bet
       dB1(iy)=-2.0*y*BB0*exp(-y*y)
1      continue
       return
       end
...
* fill matrices with zeros
       ...
       B(i,j)=0.0
1      continue
* fill diagonals of A and B
       do 2 i=1,ny
       A(i,i)=U1(i)*(-2.0-ek2)+dy*dy*(dQ1(i)-dB1(i))
       B(i,i)=-2.0-ek2
2      continue
* fill sub- and superdiagonals
       do 3 i=1,ny-1
       A(i,i+1)=U1(i)
       A(i+1,i)=U1(i+1)
       B(i,i+1)=1.0
       B(i+1,i)=1.0
3      continue
* boundary conditions in y=0 and y=ymax
       if(bcv0) then
       A(1,2)=2.0*A(1,2)
       B(1,2)=2.0*B(1,2)
       else
       ...
       A(ny,ny-1)=0.0
       B(ny,ny-1)=0.0
* other submatrices of A and B
       do 4 i=1,ny
* que se passe-t-il si on supprime ce terme de couplage ?
c       A(i,i+ny)=0.0
       A(i,i+ny)=-U1(i)*dy*dy
c       A(i+ny,i)=0.0
       A(i+ny,i)=dB1(i)
